import numpy as np
import librosa as lb # Keeping your alias
...
def format_shape(data, target_height=128, target_width=1000):
    ...
    # FIX: Added 1e-6 to prevent Division by Zero on silent audio
...
def audio_to_image(file=None, max_size=1000, y=None, sr=22050):
    if file is not None:
        if y is None:
            # FIX: Use 'lb' to match your import alias
            y, sr = lb.load(file, sr=22050)
    
    y = np.asarray(y, dtype=np.float32)

    # FIX: Use 'lb' for features
